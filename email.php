...
<html>
<head>
<title>Patient Email  Reminder (tmr) </title>
<style> 
...
<?php //echo $_SESSION["gethkid"];?>
        ...
        <h4 align="left">Patient Injection Email Reminder (Tomorrow)</h4><hr><br>
		
          <a href="emailapi.php"><button name="send" type="submit" class="btn btn-secondary">Send Email Reminder</button></a>
          <br>
          </p>
          </div>
          </section>
...
<th>Subject</th><th>Email</th><th>Message</th>
</tr>
...
patientvaccinedetail.nthinjection,patientvaccinedetail.date,patientvaccinedetail.nextdate,patientvaccinedetail.skip,patientvaccinedetail.language, patientvaccinedetail.traditionalmessage, 
 patientvaccinedetail.engmessage,patientvaccinedetail.simplifiedmessage,patientvaccinedetail.language, patients.email, patients.email
FROM patientvaccinedetail
 JOIN patients ON patientvaccinedetail.patientid=patients.patientNo
WHERE DATEDIFF(NOW(), nextdate) = -1 AND (language ='繁體' OR language ='ENG' OR language ='简体') AND smsorcall = 'Email'");?>
<?php
...
<td width="200px">
 <textarea name="msg" style="border: none"><?php if($row['language'] == '繁體')
     ...
     {
      $specific[] = ["email" => $row["email"],
                     "subject" => "Vaccine Injection Reminder (Tomorrow)",
