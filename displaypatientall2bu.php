...
<!-- The Wrapper box that wrap the whole main content-->
  ...
  <br><h4>Patient Injection Record</h4><hr>
  
  	<a href="javascript:history.go(-1)"><img src="images/backback.png" alt="back button" width="35px"></a><br>
...
$result = mysql_query("SELECT DISTINCT * FROM patientvaccinedetail WHERE patientid = '" . $_GET['patientid']."' AND vaccineid = '" . $_GET['vaccineid']."'")	or die(mysql_error());?>            <!-- display data in an editable table-->
            ...
            <tr>
              ...
              <td><textarea style="border: none" rows="2" cols="18" name="vaccinename3[]"><?php echo $row['vaccinename3'];?></textarea> </td>
...
<option value="ENG" <?php if($row['language'] == 'ENG') echo 'selected="selected"'; ?> >ENG</option>
	...
	<?php 
	   ...
	   <td><textarea rows="3" cols="13" class="form-control url" name="traditionalmessage[]" data-value="<?php echo $row['treatmentname1'].'下一個注射期為';?>"><?php echo $row['traditionalmessage'];?></textarea> </td><?php }?>
...
<option value="closed" <?php if ($row['status'] == 'closed') echo 'selected="selected"';?> >closed</option>
			  ...
			  <!--This patientid column is a hidden field-->
			      <td><input style="border: none" type="hidden" size="8" name="patientid[]"  value="<?php echo $row['patientid'] ?>"></td>
