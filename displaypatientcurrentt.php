...
<!-- Nav Bar -->
  ...
  &nbsp;&nbsp;<h4 class="display-5"> Patient Injection Record</h4><hr>
  
  <form action="newpatient2.php" method="post">
  <input type="hidden" name="pidpid" value="<?php echo $_SESSION['current'];?>">
 
 
  <input type="image" src="images/addadd.png" border="0" width="38px" title="Add new injection" alt="Submit" />
  ...
  <input type="image" src="images/old.png" border="0" width="35px" title="View inactive injection record" alt="Submit" />
...
$result = mysql_query("SELECT *FROM patients
WHERE patientNo = '" . $_SESSION['current'] . "'");?>
...
FROM patientvaccinedetail WHERE patientid = '" . $_SESSION['current'] . "' AND status= 'open' Group by vaccineid")	or die(mysql_error());?>            <!-- display data in an editable table-->
...
<!--============================-->
		       <td> <a href="displaypatientcurrent1t.php?vaccineid=<?php echo "$row[vaccineid]";?>&patientid=<?php echo "$_SESSION[getvaccine]"; ?>"><?php echo $row['vaccinename3'];?></a></td>
