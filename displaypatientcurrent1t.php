...
<br>
	<hr>
        ...
        <!-- ***********  The Select dropdown menu that allow user to pick each Patient Specific injection type ************* -->
	       ...
	       <!-- ***********  Show the patient personal info from the patient table ************* -->
	<a href="javascript:history.go(-1)"><img src="images/backback.png" width="35px"></a>
...
if (isset($_POST['submit'])){
 ...
 mysql_query("UPDATE patientvaccinedetail SET patientid ='$data1',patienthkid ='$data17',vaccineid='$data2',vaccinename3='$data5', totalnoofinjection='$data6',
		...
		WHERE id=$id")	 or die(mysql_error());
	    	...
	    	header("Location: displaypatientcurrentt.php");
		}
        }
      		?>
          <?php
...
$result = mysql_query("SELECT DISTINCT * FROM patientvaccinedetail WHERE patientid = '" . $_GET['patientid']."' AND vaccineid = '" . $_GET['vaccineid']."'")	or die(mysql_error());?>            <!-- display data in an editable table-->
            ...
            <tr>
              ...
              <td><input class="form-control" type="text" size="3" name="patientid[]"  value="<?php echo $row['patientid'] ?>"></td>
              <td><input class="form-control" type="text" size="3" name="patienthkid[]"  value="<?php echo $row['patienthkid'] ?>"></td>
              <td><input class="form-control" type="text" size="3" name="vaccineid[]" value="<?php echo $row['vaccineid'] ?>"></td>
              <td><textarea rows="2" cols="15" name="vaccinename3[]"><?php echo $row['vaccinename3'];?></textarea> </td>
